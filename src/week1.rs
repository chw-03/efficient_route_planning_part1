...
pub fn cost_calc(head: Coordinate, tail: Coordinate, way_tags: Vec<Tag>) -> f64 {
    let road_type: &str = &(way_tags.into_iter().find(|k| k.key.eq(&"highway")).unwrap().value);
    ...
    let distance = f64::sqrt(f64::powi(head.lat() - tail.lat(), 2) 
                            + f64::powi(head.lon() - tail.lon(), 2));
    distance / (kmh as f64)
}
...
impl RoadNetwork {
    pub fn new(path: &str) -> Self {
        ...
        let nodes = osm.nodes;
        let ways = osm.ways;
        let mut outgoing_arcs: Vec<Vec<Edge>> = Vec::new();
        for n in 0..nodes.len() {
            let adjacent_edges: Vec<Edge> = ways.iter().map(|way| {
                let pos = way.refs.iter().position(|&m| m == nodes[n].id);
                if pos.is_some() {
                    let index = way.refs[pos.unwrap() + 1];
                    let head = nodes.iter().find(|node: &&Node| node.id == index).unwrap().clone();
                    let cost =  cost_calc(head.coordinate, nodes[n].coordinate, way.meta.tags.clone());
                    Some(Edge {head, cost})
                }
                else {None}
...
mod tests {
    ...
    fn run_the_thing() {
        ...
        print!("#nodes: {}, #arcs: {}", result.nodes.len(), result.outgoing_arcs.len());
    }
}
