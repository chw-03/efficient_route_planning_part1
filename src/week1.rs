...
fn speed_calc(highway: &str) -> Option<u16> {   //calculates speed of highway based on given values
    match highway {
        "motorway" => Some(110),
        "trunk" => Some(110),
        "primary" => Some(70),
...
impl RoadNetwork {
    pub fn new(nodes: HashMap<i64, Node>, ways: Vec<Way>) -> Self {     //init new RoadNetwork based on results from reading .pbf file
        ...
        for way in ways {
            ...
            for i in 0..way.refs.len() - 1 {
                ...
                if let (Some(tail), Some(head)) = (tail, head) {
                    let cost = ((i64::pow((head.lat - tail.lat) * 111229, 2) as f64 / f64::powi(10.0, 4))
                                     + (i64::pow((head.lon - tail.lon) * 71695, 2) as f64 / f64::powi(10.0, 4))
                                    .sqrt() / (way.speed as f64)) as u16;  // km / kmph == h (hours)
                    edges
                        .entry(tail_id)
                        .and_modify(|inner| {
